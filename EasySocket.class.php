...
class EasySocket extends WebSocket {
    private $users_login    = array();
    private $debug_login    = false;
    private $user_send      = null;
    
    private function setLogin($token, $token_socket) {
        $this->users_login[$token_socket]['token'] = $token;
        $this->sending("< Login success.");
        if($this->debug_login) {
            print_r($this->users_login);
        }
        return true;
    }
    
    private function disconnectLogin($token_socket) {
        //unset($this->users_login[$token_socket]['token']);
    }
    
    private function isLogin($token_socket) {
        if(isset($this->users_login[$token_socket]['token']))
            return true;
        else
            return false;
    }
    
    private function setUserSend($user) { 
        $this->user_send = $user; 
    }
    
    private function getUserSend() { 
        return $this->user_send; 
    }
    
    public function sending($message) { 
        $this->send($this->getUserSend()->socket, $message); 
    }
    
    private function getToken() { 
        return $this->users_login[$this->getUserSend()->id]['token']; 
    }
    
    public function disconnect($socket) {
        parent::disconnect($socket);
        if($this->isLogin($this->getUserSend()->id)) {
            $this->say("Disconnect ".$this->getToken());
            $this->disconnectLogin($this->getUserSend()->id);
        }
    }
    
    function process($user, $json){
        $this->setUserSend($user);
        $array = json_decode($json, true);
        
        if(!$this->isLogin($user->id)) {
            if(isset($array['login'])) {
                $this->setLogin($array['login'], $user->id);
            } else {
                $this->sending("< Please login your session.");
            }
        } else {
            if($this->debug_login) { $this->say("[ ".$this->getToken()." ]: ". $json); }
            $this->result($this->getToken(), $json);
        }
    }
}